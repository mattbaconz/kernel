...
describe('Kernel release-readiness workflow', () => {
  test('runs the core CLI workflow in a temporary repository', async () => {
    ...
    const skillGenerate = await runCli(
      ['skill', 'generate', '--docs-vault', 'kernel_obsidian_vault', '--set', 'lint-ready', '--json'],
      rootDir
    );
...
async function createReleaseFixture(): Promise<string> {
  ...
  await cp(
    join(repoRoot, 'tests', 'fixtures', 'skill-generate-basic', 'kernel_obsidian_vault'),
    join(rootDir, 'kernel_obsidian_vault'),
    { recursive: true }
  );
  return rootDir;
}
